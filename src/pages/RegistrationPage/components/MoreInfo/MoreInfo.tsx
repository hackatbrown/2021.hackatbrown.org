...
> {
  constructor(props: MoreInfoProps) {
    ...
    this.state = {
      ...
      raceText: this.convertSelection(
        "race",
        this.props.currentSelected["race"]
      ),
      genderText: this.convertSelection(
        "gender",
        this.props.currentSelected["gender"]
      ),
      howFindOutText: this.convertSelection(
        "findout",
        this.props.currentSelected["findout"]
      ),
      genderArrowTransform: { transform: "none" },
      raceArrowTransform: { transform: "none" },
      findoutArrowTransform: { transform: "none" },
      racePreferNot: this.props.currentSelected["race"].includes(
        "race_prefer_not"
      ),
      genderPreferNot: this.props.currentSelected["gender"].includes(
        "gender_prefer_not"
      ),
    ...
    switch (id) {
      ...
      case "gender": {
        if (selected.length == 0) {
          ...
          let converted = selected.map(
            (x) => gendersLabels[genders.indexOf(x)]
          );
          return converted.join(", ");
        }
      }
      case "findout": {
        if (selected.length == 0) {
          ...
          let converted = selected.map(
            (x) => findoutsLabels[findouts.indexOf(x)]
          );
          return converted.join(", ");
        }
      }
  ...
  raceControl = (event: any) => {
    if (event.currentTarget.id == "raceButton" || this.state.raceOpened) {
      this.setState({
        ...
        raceArrowTransform: this.state.raceOpened
          ? { transform: "none" }
  ...
  genderControl = (event: any) => {
    if (event.currentTarget.id == "genderButton" || this.state.genderOpened) {
      this.setState({
        ...
        genderArrowTransform: this.state.genderOpened
          ? { transform: "none" }
          : { transform: "rotate(180deg)" },
      });
    }
  ...
  howFindOutControl = (event: any) => {
    if (
      event.currentTarget.id == "howFindOutButton" ||
      ...
      this.setState({
        ...
        findoutArrowTransform: this.state.howFindOutOpened
          ? { transform: "none" }
          : { transform: "rotate(180deg)" },
      });
    }
  ...
  render() {
    return (
      <div className="more-info">
        <p id="description-more-info">
          We ask the following to know more about who is attending our event.
        </p>
        ...
        <form>
          <ClickAwayListener onClickAway={this.raceControl}>
            <div className="raceDiv">
              <DropButton
                style={this.dropButtonStyles}
                id="raceButton"
                onClick={this.raceControl}
              >
                {this.state.raceText}
                <svg
                  style={this.state.raceArrowTransform}
                  className="dropArrow"
                  width="13"
                  height="12"
                  viewBox="0 0 13 12"
                ...
                <div className="form-popup">
                    ...
                    return (
                      <div>
                        <FormControlLabel
                          control={
                            <Checkbox
                              disabled={this.state.racePreferNot}
                              style={{ color: "white" }}
                              checked={this.isChecked("race", value)}
                              id="race"
                              value={value}
                            />
                          }
                  ...
                  <div>
                    <FormControlLabel
                      control={
                        <Checkbox
                          onClick={this.disableRestRace}
                          style={{ color: "white" }}
                          checked={this.isChecked("race", "race_prefer_not")}
                          id="race"
                          value={"race_prefer_not"}
                        />
                      }
          ...
          <ClickAwayListener onClickAway={this.genderControl}>
            <div className="genderDiv">
              <DropButton
                style={this.dropButtonStyles}
                id="genderButton"
                onClick={this.genderControl}
              >
                {this.state.genderText}
                <svg
                  style={this.state.genderArrowTransform}
                  className="dropArrow"
                  width="13"
                  height="12"
                  viewBox="0 0 13 12"
                ...
                <div className="form-popup">
                    ...
                    return (
                      <div>
                        <FormControlLabel
                          control={
                            <Checkbox
                              disabled={this.state.genderPreferNot}
                              style={{ color: "white" }}
                              checked={this.isChecked("gender", value)}
                              id="gender"
                              value={value}
                            />
                          }
                  ...
                  <div>
                    <FormControlLabel
                      control={
                        <Checkbox
                          onClick={this.disableRestGender}
                          style={{ color: "white" }}
          ...
          <ClickAwayListener onClickAway={this.howFindOutControl}>
            <div className="howFindOutDiv">
              <DropButton
                style={this.dropButtonStyles}
                id="howFindOutButton"
                onClick={this.howFindOutControl}
              >
                {this.state.howFindOutText}
                <svg
                  style={this.state.findoutArrowTransform}
                  className="dropArrow"
                  width="13"
                  height="12"
                  viewBox="0 0 13 12"
                      ...
                      <div>
                        <FormControlLabel
                          control={
                            <Checkbox
                              style={{ color: "white" }}
                              checked={this.isChecked("findout", value)}
                              id="findout"
                              value={value}
                            />
                          }
